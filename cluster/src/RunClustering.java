...
public class RunClustering {
    // This class reads the neccesay parameters and runs the
    // clusteringAlgorithms
    // Students don't have to edit this to complete their assignments

    public static ClusteringAlgorithm
        ca; // handle to the actual clustering algorithm
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 4) { // use user defined file names
            ...
            System.out.println("No files where defined (java runClustering " +
                               "[traindata, testdata, requests, clients]), " +
                               "using defaults");
            readData();
        }
        ...
        while (true) {
            if (startUp) {
                // Ask for algoritm ans its apropriate parameters. Algorithm
                // gets copies of the datavectors (fail-prove)
                initializeAlgorithm();

                // Training
                System.out.print("Perform the actual training! (hit enter)");
                // You wait for authorisation because in real
                // applications,training and or testing may take days.
            ...
            // Show of results
            startUp = showResult(); // ask what information should be shown. (Or
                                    // train another algorithm)
        }
    }
    ...
    public static int chooseResult() {
        int resultID = 0;
        while (true) {
            System.out.print("Show output printTest(1), vector members(2), " +
                             "vector prototypes(3), Quit(4) or set " +
                             "prefetchThreshold(5)? ");
            String line = "";
            try {
    ...
    public static void initializeAlgorithm() {
        // determine which algorithm is requested (chooseAlgorithm), and ask for
    ...
    public static void kmeansInit() {
        ...
        // java doesn't have a 'const' specifier, so for safety objects are
        // copied
        ca = new KMeans(k, new Vector<float[]>(trainData),
                        new Vector<float[]>(testData), dim);
    }
    ...
    public static void leaderFollowerInit() {
        ...
        // java doesn't have a 'const' specifier, so for safety objects are
        // passed by reference
        ca = new LeaderFollower(dis, new Vector<float[]>(trainData),
                                new Vector<float[]>(testData), dim);
    }
    ...
    public static void kohonenInit() {
        ...
        // java doesn't have a 'const' specifier, so for safety objects are
        // passed by reference
        ca = new Kohonen(n, epochs, new Vector<float[]>(trainData),
                         new Vector<float[]>(testData), dim);
    }
        ...
        case 5:
            try {
                ...
                double prefetchThreshold =
                    (new Double(in.readLine())).doubleValue();
                ca.setPrefetchThreshold(prefetchThreshold);
                System.out.println(
                    "Testing algorithm with prefetchThreshold = " +
                    prefetchThreshold + "...");
                ca.test();
            } catch (Exception e) {
            }
    ...
    public static void readData(String trainFileName, String testFileName,
                                String requestFileName, String clientFileName) {
    ...
    private static void readTrainData(String trainFileName) {
        try {
            BufferedReader br =
                new BufferedReader(new FileReader(trainFileName));
            String line = "";
    ...
    private static void readRequests(String requestsFileName) {
        try {
            BufferedReader br =
                new BufferedReader(new FileReader(requestsFileName));
            String line = "";
    ...
    private static void readClients(String clientsFileName) {
        try {
            BufferedReader br =
                new BufferedReader(new FileReader(clientsFileName));
            String line = "";
